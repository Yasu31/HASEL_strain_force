...
for k = 1:length(files)
    ...
    legend_text = extractLabelFromFileame(filename);
...
% Adjust variable names if they contain spaces or special characters
...
if isempty(Force_col) || isempty(Strain_mean_col)
    error('Cannot find "Force" or "Strain_mean" columns in file %s', files(k).name);
end
...
function label = extractLabelFromFileame(filename)
